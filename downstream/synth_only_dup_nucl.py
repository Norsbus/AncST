...
def run_blast_for_chain(args_tuple):
    ...
    # Prepare FASTA files
    fna1 = f"{output_dir}/to_comp/{i}_1.fna"
    fna2 = f"{output_dir}/to_comp/{i}_2.fna"
    ...
    SeqIO.write(seqs1, fna1, "fasta")
    SeqIO.write(seqs2, fna2, "fasta")

    # Run BLAST in both directions
    run([blastn_path, "-query", fna1, "-subject", fna2, "-outfmt", "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore", "-out", blast_out_1, "-num_threads", "1","-word_size","4"], capture_output=True)
    run([blastn_path, "-query", fna2, "-subject", fna1, "-outfmt", "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore", "-out", blast_out_2, "-num_threads", "1","-word_size","4"], capture_output=True)
