...
def get_coords(coords_to_exclude,aligned_path,query_org,target_orgs,regions,first=0):
    ...
    # aligned/{org} now contains syntenic markers (added by finalize_aligned)
        ...
        for i in local_iss:
            ...
            for org in target_orgs:
                if org in bib['matches'] and bib['matches'][org]['meta']['multiple matches out of tolerance range'] != 1:
                    if 'matches' in bib['matches'][org] and len(bib['matches'][org]['matches']) > 0:
                    ...
                    if 'dups_matches' in bib['matches'][org] and len(bib['matches'][org]['dups_matches']) > 1 and 'syntenic' in bib['matches'][org]['dups_matches'] and len(bib['matches'][org]['dups_matches']['syntenic']) > 0:
                        matches = bib['matches'][org]['dups_matches']['syntenic']
                        if i not in matching_anchors:
                            matching_anchors[i] = {}
                        if org not in matching_anchors[i]:
                            matching_anchors[i][org] = set()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--include_seed_regions', action='store_true',
        help="Append all input seed positions to syntenic_regions_succinct unconditionally, "
             "regardless of whether any CAncST anchors were found nearby. "
             "DEFAULT (flag absent): only regions supported by anchor-cluster evidence are written — "
             "seeds with no anchors in their neighbourhood will produce NO output row. "
             "Enabling this flag ensures every input element is represented in the output, "
             "but the added SEED rows carry NO syntenic anchor evidence.")
    parser.add_argument('--seed_region_margin', type=int, default=None,
        help="Padding (bp) added around each seed position when writing SEED rows. "
             "Defaults to the value of --region_buffer if not specified.")
    ...
    if args.include_seed_regions:
        ...
        with open('syntenic_regions_succinct', 'a') as f:
            for org, chromo, start, end, ori, name in all_elements:
                start = int(start)
                end = int(end)
                if org not in small_meta:
                    continue
                seqids, seqlen = small_meta[org]
                if chromo not in seqids:
                    continue
                ...
                # Convert seed to absolute coords for overlap check against anchor regions
                abs_start = start + le
                abs_end   = end   + le
                mid_abs = (abs_start + abs_end) // 2

                # Skip if seed's midpoint already falls within an existing anchor-based region
                # (boundaries extended by region_buffer, matching what was written above)
                covered = any(
                    rs - region_buffer <= mid_abs <= re + region_buffer
                    for ident, (rs, re) in global_regions.get(org, {}).items()
                )
                if covered:
                    continue

                padded_start = max(0, start - seed_margin)
                padded_end   = min(len_chr, end + seed_margin)
                label = f'SEED REGION (seed ±{seed_margin} bp included because of --include_seed_regions flag but without anchors)'
                f.write(f'{org}\t{chromo}\t{padded_start}\t{padded_end}\t{label}\n')
