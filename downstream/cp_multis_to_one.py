...
run('cp -r README_utils replace_names.py mapping get_mapping.py clasp.x find_pot_homologs.py orgs out/utils',shell=True)
run('cp -r README_dialign dialign.homology make_anc.py out/dialign',shell=True)
run('cp -r README_custom draw_simple.py draw_verbose.py pairwise_alignments_table out/custom',shell=True)
#run('cp -r README_iadhore adhore_config adhore_families.table adhore_pairwise.table adhore_gene_lists out/i-ADHoRe',shell=True)
