...
parser.add_argument('--verify', action='store_true', help='Save drawn coords to tmp_drawing_verification/ for independent verification')
...
for sog_idx, (sog_id, sog_genes) in enumerate(sogs.items()):
    ...
    # Save verification data if --verify
    if args.verify:
        verify_dir = f'{output_path}/tmp_drawing_verification/{sog_id}'
        os.makedirs(verify_dir, exist_ok=True)

        # genes.tsv - re-compute plot coords for each gene
        with open(f'{verify_dir}/genes.tsv', 'w') as vf:
            vf.write('gene_id\tspecies\tchrom\tgenomic_start\tgenomic_end\tstrand\t'
                     'plot_start\tplot_end\tplot_strand\tshift\tgenome_size\t'
                     'track_orientation\ttrack_name\tsog_id\n')
            for org, chromo in org_chromo_order:
                if org not in sog_coords or chromo not in sog_coords[org]:
                    continue
                start_range = min(int(g[0]) for g in sog_coords[org][chromo]) - margin
                end_range = max(int(g[1]) for g in sog_coords[org][chromo]) + margin
                if start_range < 0:
                    start_range = 0
                shift = start_range
                genome_size = end_range - start_range
                name = (org + ' ' + chromo + ' ' + sog_ori[(org,chromo)]).replace('_', ' ')

                for start_l, end_l, strand, hit_name in sog_coords[org][chromo]:
                    start_l, end_l = int(start_l), int(end_l)
                    if end_l < start_range or start_l > end_range:
                        continue
                    plot_start = start_l - shift
                    plot_end = end_l - shift
                    plot_strand = strand
                    if sog_ori[(org,chromo)] == 'reverse':
                        plot_start, plot_end = turn(plot_start, plot_end, genome_sizes.get(name, [(0, genome_size)]))
                        plot_strand = -strand
                    vf.write(f'{hit_name}\t{org}\t{chromo}\t{start_l}\t{end_l}\t{strand}\t'
                             f'{plot_start}\t{plot_end}\t{plot_strand}\t{shift}\t{genome_size}\t'
                             f'{sog_ori[(org,chromo)]}\t{name}\t{sog_id}\n')

        # tracks.tsv
        with open(f'{verify_dir}/tracks.tsv', 'w') as vf:
            vf.write('track_name\tspecies\tchrom\tshift\tgenome_size\torientation\tsegments\n')
            for org, chromo in org_chromo_order:
                if org not in sog_coords or chromo not in sog_coords[org]:
                    continue
                start_range = min(int(g[0]) for g in sog_coords[org][chromo]) - margin
                ...
                if start_range < 0:
                    start_range = 0
                name = (org + ' ' + chromo + ' ' + sog_ori[(org,chromo)]).replace('_', ' ')
                segs = genome_sizes.get(name, [])
                vf.write(f'{name}\t{org}\t{chromo}\t{start_range}\t{end_range - start_range}\t'
                         f'{sog_ori[(org,chromo)]}\t{segs}\n')

        # links.tsv - empty for now (link verification is complex in this script)
        with open(f'{verify_dir}/links.tsv', 'w') as vf:
            vf.write('org1\tchr1\tgenomic_start1\tgenomic_end1\tplot_start1\tplot_end1\t'
                     'org2\tchr2\tgenomic_start2\tgenomic_end2\tplot_start2\tplot_end2\t'
                     'alignment_orientation\ttrack_ori1\ttrack_ori2\tshift1\tshift2\t'
                     'genome_size1\tgenome_size2\n')

        print(f'  Verification data saved to {verify_dir}/')
