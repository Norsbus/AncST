...
def write_unassigned_genes(all_input_genes, singletons, filtered, gene_species, overlap_filtered, output_path):
    """
    Write SynOrthogroups_UnassignedGenes.tsv.

    Includes:
        - Overlap-filtered genes (removed during GFF parsing due to overlapping with larger gene)
        - Filtered genes (removed during pipeline - not in synteny chains)
    ...
    with open(output_path, 'w') as f:
        ...
        # Overlap-filtered genes (filtered during GFF/FAA parsing)
        for species_name in sorted(overlap_filtered.keys()):
            for gene in sorted(overlap_filtered[species_name].keys()):
                f.write(f"{gene}\t{species_name}\tfiltered_out_due_to_overlap_with_larger_seq\n")
...
def write_statistics_overall(all_input_genes, sogs, singletons, filtered, gene_species, overlap_filtered, output_path):
    """
    ...
    total_overlap_filtered = sum(len(genes) for genes in overlap_filtered.values())
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f"Total input genes (in parsed GFF)\t{total_input}\n")
        f.write(f"Genes filtered during GFF parsing (overlaps)\t{total_overlap_filtered}\n")
        f.write(f"Genes in SynOrthogroups\t{genes_in_sogs}\n")
        f.write(f"Genes unassigned (not in synteny chains)\t{genes_unassigned_filtered}\n")
        f.write(f"Genes unassigned (single-species singletons)\t{genes_unassigned_singletons}\n")
...
def write_statistics_per_species(all_input_genes, sogs, singletons, filtered,
                                 gene_species, overlap_filtered, species_list, output_path):
    """
    ...
    # Count overlap-filtered genes per species
    species_overlap_filtered = defaultdict(int)
    for species_name, genes in overlap_filtered.items():
        species_overlap_filtered[species_name] = len(genes)
    ...
    with open(output_path, 'w') as f:
        f.write("Species\tInput_Genes\tOverlap_Filtered\tIn_SynOrthogroups\tUnassigned\tPercentage\tSingleCopy_SOGs\n")

        for species in species_list:
            input_count = species_input_counts[species]
            overlap_count = species_overlap_filtered[species]
            ...
            f.write(f"{species}\t{input_count}\t{overlap_count}\t{in_sogs}\t{unassigned}\t"
...
def write_pickle_format(sogs, all_input_genes, singletons, filtered,
                        gene_species, overlap_filtered, parsed_gff, graph_version, output_path):
    """
    ...
    unassigned = {}
    # Add overlap-filtered genes
    for species_name, genes in overlap_filtered.items():
        for gene in genes:
            unassigned[gene] = 'filtered_out_due_to_overlap_with_larger_seq'
    # Add other filtered genes
    for gene in filtered:
...
def export_synorthogroups_version(output_dir, graph_version):
    ...
    # Extract overlap-filtered genes from metadata
    overlap_filtered_genes = metadata.get('overlap_filtered_genes', {})
    total_overlap_filtered = sum(len(genes) for genes in overlap_filtered_genes.values())
    ...
    print(f"  Filtered genes (not in synteny chains): {len(filtered)}")
    print(f"  Overlap-filtered genes (during GFF parsing): {total_overlap_filtered}")
    ...
    write_unassigned_genes(
        all_input_genes, singletons, filtered, gene_species, overlap_filtered_genes,
        f"{synortho_dir}/SynOrthogroups_UnassignedGenes.tsv"
    )

    write_statistics_overall(
        all_input_genes, sogs, singletons, filtered, gene_species, overlap_filtered_genes,
        f"{synortho_dir}/Statistics_Overall.tsv"
    )

    write_statistics_per_species(
        all_input_genes, sogs, singletons, filtered, gene_species, overlap_filtered_genes, species_list,
    ...
    write_pickle_format(
        sogs, all_input_genes, singletons, filtered, gene_species, overlap_filtered_genes, parsed_gff, graph_version,
