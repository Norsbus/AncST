...
try:
    # tralda >= 2.0 (lowercase module names)
...
# GENOMIC ALIGNMENT MODULE (align_genomic_simple_tuples) - SAME AS NUCL VERSION
        ...
        while i > 0 or j > 0:
            ...
            if op_type == MATCH:
                ...
                for idx in range(p - 1, -1, -1):
                    aligned1.append(seq1_dups[idx])
                    aligned2.append(elem2 if idx == p - 1 else "DUP")
                ...
                for idx in range(q - 1, -1, -1):
                    aligned1.append(elem1 if idx == q - 1 else "DUP")
                    aligned2.append(seq2_dups[idx])
...
parser = argparse.ArgumentParser(description='Simple Synthology Pipeline')
...
parser.add_argument('--homology_threshold', type=int, default=30, help="BLAST bit score threshold")
parser.add_argument('--annotation_dir', type=str, default="annotations_for_synthology", help='directory with gff/ and proteins/ subdirs')
...
parser.add_argument('--cograph-runs', type=int, default=10, help='number of random restarts for cograph editing (default: 10)')
parser.add_argument('--edit-to-cograph-before-list-alignments-no-new-edges', action='store_true',
                    help='run cograph editing on per-chain graphs BEFORE pairwise alignments (no new edges allowed)')
parser.add_argument('--edit-to-cograph-before-list-alignments-with-new-edges', action='store_true',
                    help='run cograph editing on per-chain graphs BEFORE pairwise alignments (new cross-species edges allowed)')
parser.add_argument('--no-cache', action='store_true',
                    help='ignore all cached results and recompute everything from scratch')
args = parser.parse_args()

if args.edit_to_cograph_before_list_alignments_no_new_edges and args.edit_to_cograph_before_list_alignments_with_new_edges:
    parser.error("Cannot use both --edit-to-cograph-before-list-alignments-no-new-edges and --edit-to-cograph-before-list-alignments-with-new-edges")

# Determine pre-alignment cograph editing mode
if args.edit_to_cograph_before_list_alignments_no_new_edges:
    PRE_EDIT_MODE = 'no_new_edges'
elif args.edit_to_cograph_before_list_alignments_with_new_edges:
    PRE_EDIT_MODE = 'with_new_edges'
else:
    PRE_EDIT_MODE = None
...
FAA_DIR = os.path.join(args.annotation_dir, 'proteins/')
NO_CACHE = args.no_cache
NEW_BLAST = args.new_blast == 'y' or NO_CACHE
...
def parse_blast_output(blast_file):
    ...
    with open(blast_file) as f:
        for line in f:
            ...
            if len(cols) >= 7:
                hits.append((cols[0], cols[1], float(cols[2])))
    return hits
...
def run_blast_for_chain(args_tuple):
    ...
    if not new_blast:
        if use_clasp:
            ...
            clasp_files = [f'{output_dir}/clasp_out/{i}_{suffix}' for suffix in ['1', '2']]
    ...
    faa_path1 = f"{faa_dir}/{chain['org1']}.faa"
    faa_path2 = f"{faa_dir}/{chain['org2']}.faa"

    seqs1 = []
    for record in SeqIO.parse(faa_path1, "fasta"):
        if record.id.split()[0] in pids1:
            seqs1.append(record)

    seqs2 = []
    for record in SeqIO.parse(faa_path2, "fasta"):
    ...
    if use_clasp:
        ...
        clasp_in_2 = f"{output_dir}/clasp_in/{i}_2"
        clasp_out_1 = f"{output_dir}/clasp_out/{i}_1"
        clasp_out_2 = f"{output_dir}/clasp_out/{i}_2"

        run(f'cp {blast_out_1} {clasp_in_1}', shell=True)
        ...
        # Parse CLASP outputs
        hits = []
        hits.extend(parse_clasp_output(clasp_out_1))
...
def build_graph_from_blast(args_tuple):
    i, chain, blast_hits, blast_threshold, pre_edit_mode, cograph_runs = args_tuple
    ...
    if pre_edit_mode == 'no_new_edges':
        original_edges = set(G.edges())
        def is_forbidden(u, v):
            return (u, v) not in original_edges and (v, u) not in original_edges
        G = edit_to_cograph(G, run_number=cograph_runs, forbidden_edge_checker=is_forbidden)
    elif pre_edit_mode == 'with_new_edges':
        # Allow new cross-species edges, forbid same-species edges
        genes1_set = {pid for s, e, strand, pid in chain['genes1']}
        genes2_set = {pid for s, e, strand, pid in chain['genes2']}
        def is_forbidden(u, v):
            if u in genes1_set and v in genes1_set:
                return True
            if u in genes2_set and v in genes2_set:
                return True
            return False
        G = edit_to_cograph(G, run_number=cograph_runs, forbidden_edge_checker=is_forbidden)
    # else pre_edit_mode is None: skip cograph editing
...
if __name__ == '__main__':
    ...
    filtered_genes_pickle = f"{OUTPUT_DIR}/overlap_filtered_genes.pickle"

    if not NO_CACHE and os.path.isfile(parsed_path):
        print("Loading cached annotations...")
        with open(parsed_path, 'rb') as f:
            parsed = pickle.load(f)

        # Load overlap filtered genes if available
        if not NO_CACHE and os.path.isfile(filtered_genes_pickle):
            with open(filtered_genes_pickle, 'rb') as f:
                overlap_filtered_genes = pickle.load(f)
        else:
            overlap_filtered_genes = {}
    else:
        ...
        for gff_file in os.listdir(GFF_DIR):
            ...
            faa_file = os.path.join(FAA_DIR, species + '.faa')

            if not os.path.exists(faa_file):
                raise FileNotFoundError(f"GFF file exists but FAA file missing: {faa_file}")
            ...
            with open(os.path.join(GFF_DIR, gff_file)) as f:
                for line in f:
                    ...
                    # Only parse CDS features (proteins correspond to coding sequences)
                    if ftype != "CDS":
                        continue
            ...
            # Track filtered genes due to overlap
            if species not in overlap_filtered_genes:
                overlap_filtered_genes[species] = {}
                    ...
                    for i, (ks, ke, kstrand, kid) in enumerate(kept):
                        if not (end < ks or start > ke):
                            ...
                            if new_len > old_len:
                                ...
                                overlap_filtered_genes[species][kid] = {
                                    'reason': 'filtered_out_due_to_overlap_with_larger_seq',
                                    'chromosome': seqid,
                                    'position': f"{ks}-{ke}",
                                    'overlaps_with': pid,
                                    'overlapping_position': f"{start}-{end}",
                                    'filtered_length': old_len,
                                    'kept_length': new_len
                                }
                                kept[i] = (start, end, strand, pid)
                                overlapping_gene = kid
                            else:
                                # Current gene is shorter - filter it out
                                overlap_filtered_genes[species][pid] = {
                                    'reason': 'filtered_out_due_to_overlap_with_larger_seq',
                                    'chromosome': seqid,
                                    'position': f"{start}-{end}",
                                    'overlaps_with': kid,
                                    'overlapping_position': f"{ks}-{ke}",
                                    'filtered_length': new_len,
                                    'kept_length': old_len
                                }
                                overlapping_gene = pid
                            break
        ...
        # Save overlap filtered genes
        filtered_genes_pickle = f"{OUTPUT_DIR}/overlap_filtered_genes.pickle"
        with open(filtered_genes_pickle, 'wb') as f:
            pickle.dump(overlap_filtered_genes, f)

        # Write human-readable filtered genes file
        filtered_genes_txt = f"{OUTPUT_DIR}/overlap_filtered_genes.txt"
        total_filtered = sum(len(genes) for genes in overlap_filtered_genes.values())

        with open(filtered_genes_txt, 'w') as f:
            f.write("# Genes filtered due to overlap with larger sequences\n")
            f.write(f"# Total filtered: {total_filtered}\n\n")

            for species in sorted(overlap_filtered_genes.keys()):
                if overlap_filtered_genes[species]:
                    f.write(f"\n{'='*80}\n")
                    f.write(f"Species: {species} ({len(overlap_filtered_genes[species])} genes filtered)\n")
                    f.write(f"{'='*80}\n\n")

                    for gene_id, info in sorted(overlap_filtered_genes[species].items()):
                        f.write(f"Gene: {gene_id}\n")
                        f.write(f"  Chromosome: {info['chromosome']}\n")
                        f.write(f"  Position: {info['position']}\n")
                        f.write(f"  Length: {info['filtered_length']}\n")
                        f.write(f"  Overlaps with: {info['overlaps_with']}\n")
                        f.write(f"  Overlapping position: {info['overlapping_position']}\n")
                        f.write(f"  Kept gene length: {info['kept_length']}\n")
                        f.write(f"  Reason: {info['reason']}\n\n")

        # Log statistics to screen
        print(f"\n  Overlap filtering summary:")
        print(f"    Total genes filtered: {total_filtered}")
        for species in sorted(overlap_filtered_genes.keys()):
            if overlap_filtered_genes[species]:
                print(f"      {species}: {len(overlap_filtered_genes[species])} genes")
        print(f"    Saved to: {filtered_genes_txt}")

    print(f"\nLoaded annotations for {len(parsed)} species")
    ...
    if not NO_CACHE and os.path.isfile(alignments_path):
    ...
    if not NO_CACHE and os.path.isfile(merged_path):
    ...
    print("\n" + "="*80)
    if PRE_EDIT_MODE is None:
        print("STEP 6: BUILD PAIRWISE GRAPHS (no pre-alignment cograph editing)")
    elif PRE_EDIT_MODE == 'no_new_edges':
        print("STEP 6: BUILD PAIRWISE GRAPHS + PRE-ALIGNMENT COGRAPH EDITING (no new edges)")
    else:
        print("STEP 6: BUILD PAIRWISE GRAPHS + PRE-ALIGNMENT COGRAPH EDITING (with new edges)")
    print("="*80)

    # Cache filename includes pre-edit mode so different configurations don't collide
    if PRE_EDIT_MODE is None:
        graphs_cache_path = f"{OUTPUT_DIR}/graphs.pickle"
    elif PRE_EDIT_MODE == 'no_new_edges':
        graphs_cache_path = f"{OUTPUT_DIR}/graphs_pre_edit_no_new.pickle"
    else:
        graphs_cache_path = f"{OUTPUT_DIR}/graphs_pre_edit_with_new.pickle"

    if not NO_CACHE and os.path.isfile(graphs_cache_path):
        print(f"Loading cached graphs from {os.path.basename(graphs_cache_path)}...")
        ...
        graph_args = [(i, chain, blast_dict[i], BLAST_THRESHOLD, PRE_EDIT_MODE, args.cograph_runs) for i, chain in enumerate(merged)]
    ...
    if PRE_EDIT_MODE is not None:
        ...
        print(f"  Total edges: {total_edges_before_cograph}")
        print(f"  (Pre-alignment cograph editing: OFF)")
    ...
    union_v1 = edit_to_cograph(union_v1, run_number=args.cograph_runs, forbidden_edge_checker=is_forbidden_only_same_species)
    ...
    union_v2 = edit_to_cograph(union_v2, run_number=args.cograph_runs, forbidden_edge_checker=is_forbidden_no_new_edges)
    ...
    graph_metadata = {
        ...
        'use_clasp': USE_CLASP,
        'num_cores': NUM_CORES,
        'overlap_filtered_genes': overlap_filtered_genes
    }
