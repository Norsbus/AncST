    ...
    if use_clasp:
        ...
        with open(f'{output_dir}/temp_coords_{org}','w') as out:
            for orientation in ['forward','reverse']:
                ...
                blast_file = f'{output_dir}/blast_out_{orientation}/{org}_regions'
                if not isfile(clasp_file):
                    # CLASP only runs when BLAST output exists and is non-empty
                    if isfile(blast_file) and os.path.getsize(blast_file) > 0:
                        raise FileNotFoundError(
                            f"ERROR: CLASP output file missing but BLAST input exists: {clasp_file}\n"
                            f"  BLAST input: {blast_file} ({os.path.getsize(blast_file)} bytes)\n"
                            f"  CLASP should have produced output. Check if CLASP ran successfully for {org} ({orientation})"
                        )
                    # No BLAST hits for this orientation — expected, skip
                    continue
