...
############################################################################
# CAncST Installation Script - Cross-Platform Edition
#
# This script:
...
log_info "CAncST Installation Starting..."
...
case "$ARCH_TYPE" in
  ...
  Please report this issue at: https://github.com/Norsbus/CAncST/issues"
        ;;
esac
...
find_and_initialize_best_conda() {
  ...
  SOLUTIONS:
    ...
    if [ -n "$conda_base" ] && [ -f "$conda_base/etc/profile.d/conda.sh" ]; then
        # Source conda shell integration (as suggested by ChatGPT + community)
...
if [ ! -f "$ENV_FILE" ]; then
  ...
  If you're in the wrong directory, cd to the CAncST root first."
fi
...
if ! command -v cmake &>/dev/null; then
  ...
  https://github.com/Norsbus/CAncST/issues"
fi
...
if [ ! -f "sdsl/lib/libsdsl.a" ]; then
    ...
    # See: https://github.com/Norsbus/CAncST/issues/... (LDFLAGS pollution bug)
...
#!/usr/bin/env bash
# CAncST Environment Activation Script
...
echo ""
echo "CAncST environment activated!"
echo ""
