...
start = time() 
run(f'rm -f {work_dir}/.snakemake/locks/*',shell=True)
run(f'snakemake --scheduler greedy --conda-frontend conda --use-conda --keep-incomplete --rerun-incomplete -s {code_dir}/snakefile_update_compared --profile {code_dir}/profile/ --config work_dir="{work_dir}" code_dir="{code_dir}" root_dir="{root}"',shell=True)
passed = time() - start
execution_times.write(f'prepping pw comparisons: {passed}\n')

start = time() 
run(f'rm -f {work_dir}/.snakemake/locks/*',shell=True)
run(f'snakemake --scheduler greedy --conda-frontend conda --use-conda --keep-incomplete --rerun-incomplete -s {code_dir}/snakefile_bcamm --profile {code_dir}/profile/ --config work_dir="{work_dir}" code_dir="{code_dir}" root_dir="{root}"',shell=True) 
passed = time() - start
