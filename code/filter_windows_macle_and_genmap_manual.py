...
def get_Cm_values(w,i):
    ...
    #tot = 0
    # make it non-cumulative and keeping orig code for backward checks
    for Cm in s_unique:
        #tot += Cms[Cm]
        global_dict[org].append((Cm,Cms[Cm]))
    return(global_dict)
...
def make_k_mers(org,idx,k,e,len_window,interval,percentile):
    ...
    for i,end in idx:
        ...
        records.append(SeqRecord(Seq(s[i:end]),id="kmer${}${}${}${}".format(i,ident,i_alt,end)))
        records_rev.append(SeqRecord(Seq(s[i:end]).reverse_complement(),id="kmer${}${}${}${}".format(i,ident,i_alt,end)))
...
# OLD pure-python version. kept for reference / regression checks. the
# vectorized get_sums below produces the identical (sums,indices) pair.

def get_sums_old(args):
    a,z,l,interval = args
    global kmer_counts
...
# vectorized replacement for get_sums_old. produces the identical (sums,indices)
# pair but as numpy int64 arrays. control flow (the find_next_valid + outer-while
# 10000-block jump past invalid windows) is preserved exactly. window sums and
# the zero-check are precomputed via chunked cumsum so the inner loop body is O(1).
# verified bit-identical to the get_sums_old reference implementation
# via a self-test (89 cases incl. edges around the 10000-block boundary).
def get_sums(args):
    a,z,l,interval = args
    global kmer_counts
    kc = kmer_counts[a:z]
    ...
    # winzero[j] == (0 in kc[j:j+l]), via cumsum of (kc==0). chunked so the
    # int64 cumsum stays bounded in memory.
    winzero = np.empty(nwin,dtype=bool)
    chunk = 100_000_000
    cstart = 0
    while cstart < nwin:
        cend = min(cstart+chunk, nwin)
        zc = np.cumsum(kc[cstart:cend+l-1] == 0, dtype=np.int64)
        m = cend - cstart
        top = zc[l-1:l-1+m]
        bottom = np.empty(m,dtype=np.int64)
        bottom[0] = 0
        bottom[1:] = zc[:m-1]
        winzero[cstart:cend] = (top - bottom) > 0
        cstart = cend

    # same j-walk as get_sums_old. when winzero[j] is True, do the same
    # 10000-block search for the next valid window. np.argmin on a winzero
    # slice replaces find_next_valid (first False == first valid offset).
    size_array_for_zero_check = 10000
    vis = []
    j = 0
    while j < nwin:
        if winzero[j]:
            ...
            jj = j
            while jj < nwin:
                sub_len = min(size_array_for_zero_check, n - jj)
                last_off = sub_len - l
                seg = winzero[jj:jj+last_off+1]
                if seg.size and not seg.all():
                    found = jj + int(np.argmin(seg))
                    break
                jj += size_array_for_zero_check
            if found is None:
                indices = np.asarray(vis,dtype=np.int64) + a
                sums = _get_window_sums_at(kc, indices - a, l)
                return(sums,indices)
            j = found
        vis.append(j)
        j += interval
...
# window sums at the given start positions (sorted ascending) via chunked cumsum.
# exact integer sums; returns int64 array. helper for get_sums.
def _get_window_sums_at(kc, idx, l):
    ...
    while cstart < nwin:
        cend = min(cstart+chunk, nwin)
        lo = int(np.searchsorted(idx, cstart, side='left'))
        hi = int(np.searchsorted(idx, cend,  side='left'))
        if hi > lo:
            seg = kc[cstart:cend+l-1]
            cs0 = np.empty(len(seg)+1,dtype=np.int64)
            cs0[0] = 0
            np.cumsum(seg, dtype=np.int64, out=cs0[1:])
            loc = idx[lo:hi] - cstart
            out[lo:hi] = cs0[loc+l] - cs0[loc]
        cstart = cend
    return(out)
...
def get_low_count_windows(org,genmap_out_file,out_file_name,k,e,len_window,interval,percentile,wanted_genome_size):
    ...
    last = int(np.partition(sums_global, how_many-1)[how_many-1])
    mask = sums_global <= last
    final_idx = set(int(i) for i in indices_global[mask])
...
def get_idx_from_macle(macle_file,window_size,pitch,percentile,global_Cm,wanted_genome_size):
    half_the_window_size = int(np.ceil(window_size/2))
    starts = []
    Cms = []
    lll = []
    n_skipped_short = 0
    # map each chromosome name to its index once, so the lookup below is O(1)
    # instead of O(number of contigs) per line; setdefault keeps the first
    # occurrence, which is exactly what seqids.index(chromo) returned
    seqid_to_idx = {}
    for iii,seqid in enumerate(seqids):
        seqid_to_idx.setdefault(seqid,iii)
    with open(f'{work_dir}/../utils/macle_out/{org}/{window_size}_{pitch}.txt') as f:
        for line in f:
            chromo,midpoint,Cm = line.strip().split()
            Cm = float(Cm)
            midpoint = int(midpoint)
            # fail loudly if macle reports a chromosome that is not in the genome
            # metadata (e.g. a name truncated by macle) rather than a bare KeyError
            if chromo not in seqid_to_idx:
                raise RuntimeError(f'macle output for {org} references chromosome '
                    f'"{chromo}" which is not in the genome metadata - sequence '
                    f'names may be truncated or the metadata is stale')
            idx_chr = seqid_to_idx[chromo]
            if idx_chr == 0:
                le = 0
            else:
                le = seqlen[idx_chr-1]
            if seqlen[idx_chr] - le < window_size:
                # contig shorter than the macle window - no window fits; counted
                # and reported after the loop so this skip is not silent
                n_skipped_short += 1
                continue
    ...
    if n_skipped_short > 0:
        print(f'get_idx_from_macle ({window_size}_{pitch}): skipped {n_skipped_short} '
            f'macle windows on contigs shorter than the window size {window_size}')
...
def combine_and_filter_new_ones(idx,bib=False):
    ...
    # Always merge overlapping/close candidates (removed percentile check)
    while i < len(idx)-1:
        print(f'processing {idx[i]}')
        idx_start = idx[i]
        if idx_start not in genmap_info and idx_start not in macle_info:
            raise RuntimeError(f'err {idx_start} not from macle nor genmap...?...')
        elif idx_start in genmap_info and idx_start in macle_info:
            len_lmers = max(genmap_info[idx_start][0],macle_info[idx_start][0])
            interval = max(genmap_info[idx_start][1],macle_info[idx_start][1])
        elif idx_start in genmap_info:
        ...
        if ident1 != ident2:
            ...
            continue
        next_i = idx[i+1]
        if next_i not in genmap_info and next_i not in macle_info:
            raise RuntimeError(f'err {next_i} not from macle nor genmap...?...')
        elif next_i in genmap_info and next_i in macle_info:
            len_lmers_next = max(genmap_info[next_i][0],macle_info[next_i][0])
        elif next_i in genmap_info:
            len_lmers_next = genmap_info[next_i][0]
        elif next_i in macle_info:
            len_lmers_next = macle_info[next_i][0]
        end2 = next_i + len_lmers_next
        ...
        while i < len(idx)-1 and (idx_end >= idx[i+1] or idx[i+1] - idx_end <= 100) and ident1 == ident2:
            ...
            if next_i not in genmap_info and next_i not in macle_info:
                ...
                len_lmers = max(genmap_info[next_i][0],macle_info[next_i][0])
                interval = max(genmap_info[next_i][1],macle_info[next_i][1])
            elif next_i in genmap_info:
                len_lmers,interval = genmap_info[next_i]
            elif next_i in macle_info:
                len_lmers,interval = macle_info[next_i]
            idx_end = max(idx_end, idx[i+1] + len_lmers)
            i += 1
    ...
    if len(new_idx) > 0 and len(idx) > 0:
        last_pos = idx[-1]
        if last_pos not in genmap_info and last_pos not in macle_info:
            raise RuntimeError(f'err {last_pos} not from macle nor genmap...?...')
        elif last_pos in genmap_info and last_pos in macle_info:
            len_lmers = max(genmap_info[last_pos][0],macle_info[last_pos][0])
        elif last_pos in genmap_info:
        ...
        if last_pos > new_idx[-1][1]:
            ...
            if last_pos == seqlen[pos]:
                pos += 1
            ident1 = seqids[pos]
            if last_pos + len_lmers > seqlen[pos] - 1:
                idx_end = seqlen[pos] - 1

            else:
                idx_end = last_pos + len_lmers
...
if __name__ == "__main__":
    ...
    macle_paras = []
    macle_params_file = f'{work_dir}/macle_params.txt'
    if os.path.isfile(macle_params_file):
        with open(macle_params_file) as f:
            for line in f:
                if org in line:
    ...
    genmap_paras = []
    genmap_params_file = f'{work_dir}/genmap_params.txt'
    if os.path.isfile(genmap_params_file):
        with open(genmap_params_file) as f:
            for line in f:
                if org in line:
    ...
    if not bib:
        ...
        to_del = set()  # Empty set on first run
    else:
        idx,to_del = combine_and_filter_new_ones(idx,bib)
        if len(idx) == 0:
            print('no new candidates')
            run(['touch', f'{work_dir}/touch/no_new_candidates_{org}'])

    # Always create to_del file (even empty on first run)
