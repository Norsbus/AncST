...
if __name__ == "__main__":
    ...
    max_to_take = int((25/100)*genome_size*max(2,(len_lmers/interval/2)))

    if len(k_e) == 0:
        print(f'filtering windows with parameters: k={k}, e={e}, len_lmers={len_lmers}, interval={interval}, percentile={percentile}')
        idx = get_idx_from_macle(infile,int(len_lmers/2),int(interval),percentile,wanted_genome_size,max_to_take)
        size = os.path.getsize(f'/scr/k80san/karl/test_macle/make_another_SENSIBLE_DATASET_WITH_MACLE_AND_GENMAP_COMBINED_was_origginally_test_overlapping_filtering_so_filter_logs_have_overlap_numbers/AncST/template/windows_low_kmers_fastas/{org}.fasta')
        left = wanted_genome_size
        fill_up_til_size = get_low_count_windows(org,infile,outfile,k,e,len_lmers,interval,percentile,left)
