...
def check_score(score,org1,i_seq_org1,start1,end1,org2,i_seq_org2,start2,end2,strict = 0,max_score = 0):
    if len(dict_org1[i_seq_org1]['regions']) == 0:
        ...
        if idx_in_scores1 >= len(dict_org1[i_seq_org1]['regions']):
            raise ValueError(f"BLAST hit start1={start1} exceeds {org1} anchor {i_seq_org1} regions max={dict_org1[i_seq_org1]['regions'][-1]} - corrupted data or mismatched candidates")
    ...
    if len(dict_org2[i_seq_org2]['regions']) == 0:
        ...
        if idx_in_scores1 >= len(dict_org2[i_seq_org2]['regions']):
            raise ValueError(f"BLAST hit start2={start2} exceeds {org2} anchor {i_seq_org2} regions max={dict_org2[i_seq_org2]['regions'][-1]} - corrupted data or mismatched candidates")
