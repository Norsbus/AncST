...
def get_low_count_windows(org,genmap_out_file,out_file_name,k,e,len_window,interval,percentile):
    ...
    for start7,end7 in se7: 
        sums_chr_7,indices_chr_7 = get_sums((start7,end7,l,interval))
        if len(sums_chr_7) < 10:
            continue
        perc = np.percentile(sums_chr_7,percentile)
        idx = np.where(sums_chr_7 < perc)[0]
        final_idx += [indices_chr_7[i] for i in idx]
    
    for start12,end12 in se12: 
        sums_chr_12,indices_chr_12 = get_sums((start12,end12,l,interval))
        if len(sums_chr_12) < 10:
            continue
        perc = np.percentile(sums_chr_12,percentile)
        idx = np.where(sums_chr_12 < perc)[0]
        final_idx += [indices_chr_12[i] for i in idx]
...
if __name__ == "__main__":
    ...
    with open(f'{work_dir}/chr_bib7','rb') as f:
        chr_bib7 = pickle.load(f)
    with open(f'{work_dir}/chr_bib12','rb') as f:
        chr_bib12 = pickle.load(f)
    ...
    for ident in chr_bib7[org]:
        idx_seqids = 'not found'
        for enu,full_id in enumerate(seqids):
            if ident in full_id:
                idx_seqids = enu
        if idx_seqids == 'not found':
            print(f'{ident} not found')
        #idx_seqids = seqids.index(ident)
        start7 = seqlen[idx_seqids-1]
        starts7.append(start7)
        end7 = seqlen[idx_seqids]
        ends7.append(end7)
        se7.append((start7,end7))

    for ident in chr_bib12[org]:
        idx_seqids = 'not found'
        for enu,full_id in enumerate(seqids):
            if ident in full_id:
                idx_seqids = enu
        if idx_seqids == 'not found':
            print(f'{ident} not found')
        #idx_seqids = seqids.index(ident)
        start12 = seqlen[idx_seqids-1]
        starts12.append(start12)
        end12 = seqlen[idx_seqids]
        ends12.append(end12)
        se12.append((start12,end12))
