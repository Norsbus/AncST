...
def get_low_count_windows(org,genmap_out_file,out_file_name,k,e,len_window,interval,percentile,left):
    l = int(len_window)
    ...
    #factor_pitch = l/interval
    lll = sorted(list(zip(sums_global,indices_global)))
    #how_many = int(left/l*factor_pitch)
    how_many = int(left/1000)
    final_idx = [xxx[1] for xxx in lll[:how_many]]
    last = lll[how_many-1][0]
    for xxx in lll[how_many:]:
        if xxx[0] <= last:
            final_idx.append(xxx[1])

    #perc = np.percentile(sums_global,percentile)
    perc_best = np.percentile(sums_global,10)
    #idx = np.where(sums_global < perc)[0]
    idx_best = np.where(sums_global < perc_best)[0]
    #final_idx = [indices_global[i] for i in idx]
    final_idx = sorted(final_idx)
    final_idx_best = [indices_global[i] for i in idx_best]
    final_idx_best = sorted(final_idx_best)


    return(final_idx,final_idx_best,[xxx[1] for xxx in lll[len(idx_best):]])
...
def get_idx_from_macle(macle_file,half_the_window_size,pitch,percentile,wanted_genome_size,max_to_take):
    starts = []
    Cms = []
    lll = []
    ...
    #threep = math.ceil(wanted_genome_size/(half_the_window_size*2))
    #print(threep)
    lll = sorted(lll,reverse=True)
    #thres = lll[threep-1][0]
    ...
    to_take = []
    for Cm in s_unique:
        if len(to_take)*half_the_window_size*2 < wanted_genome_size and len(to_take)*half_the_window_size*2 + len(unique_d[Cm])*half_the_window_size*2 < (wanted_genome_size*1.2) and len(to_take)*half_the_window_size*2 + len(unique_d[Cm])*half_the_window_size*2 < max_to_take:
            to_take += unique_d[Cm]
        else:
            break
    print('HERE')
    for xxx,yyy in sorted(unique_d.items(),reverse=True):
        print(f'{xxx} : {len(yyy)}')
    final_idx = to_take#[xxx[1] for xxx in lll if xxx[0] >= thres]
    final_idx = sorted(final_idx)

    perc_best = np.percentile(Cms,100-7)

    to_take = []
    for Cm in s_unique:
        #if len(to_take) < 10000 and len(to_take) + len(unique_d[Cm]) < 30000:
        if len(to_take)*half_the_window_size*2 < wanted_genome_size and len(to_take)*half_the_window_size*2 + len(unique_d[Cm])*half_the_window_size*2 < (wanted_genome_size):
            to_take += unique_d[Cm]
        else:
            break
    final_idx_best = to_take#[xxx[1] for xxx in lll if xxx[0] >= perc_best]
    final_idx_best = sorted(final_idx_best)
    print('number processed:')
    print(len(final_idx),len(final_idx_best))
    return(final_idx,final_idx_best)
...
#
    ...
    ss = 0
    for enu,sid in enumerate(seqids):
        print(f'{sid} : {ss} - {seqlen[enu] - 1}')
        ss = seqlen[enu]

    genome_size = os.path.getsize(f'../utils/genomes/{org}.fasta')
    wanted_genome_size = int((percentile/100)*genome_size*max(2,(len_lmers/interval/2)))
    max_to_take = int((25/100)*genome_size*min(2,(len_lmers/interval/2)))
    print(f'filtering windows with parameters: k={k}, e={e}, len_lmers={len_lmers}, interval={interval}, percentile={percentile}')
    idx1,best1 = get_idx_from_macle(infile_macle,int(len_lmers/2),int(interval),percentile,wanted_genome_size*0.67,max_to_take)
   
    idx1 = list(set(idx1))
    print('eval11 : len of idx with overlapping macle')
    print(len(idx1),1000*len(idx1))
    idx_t = combine_and_filter_new_ones(idx1)
    print('eval11 : len of idx without overlapping macle')
    print(len(idx_t))
    tot_len = 0
    for i in idx_t:
        tot_len += i[1] - i[0]
    ...
    best1 = list(set(best1))
    print('eval12 : len of best idx with overlapping macle')
    print(len(best1),1000*len(best1))
    idx_t = combine_and_filter_new_ones(best1)
    print('eval12 : len of best idx without overlapping macle')
    print(len(idx_t))
    tot_len = 0
    for i in idx_t:
        tot_len += i[1] - i[0]
    ...
    print('normal and best combined')
    idx_tt = idx1 + best1
    print(len(idx_tt),1000*len(idx_tt))
    idx_t = combine_and_filter_new_ones(idx_tt)
    print(len(idx_t))
    tot_len = 0
    for i in idx_t:
        tot_len += i[1] - i[0]
    ...
    both1 = idx1 + best1
    left = wanted_genome_size - ((len(list(set(both1)))*len_lmers))
    left = int(left)
    ...
    infile = f'{root}/utils/genmap_out/{org}/{k}_{e}.freq16'
    print(f'filtering windows with parameters: k={k}, e={e}, len_lmers={len_lmers}, interval={interval}, percentile={percentile}')
    idx2,best2,fill_up_til_size = get_low_count_windows(org,infile,outfile,k,e,len_lmers,interval,percentile,left)

    idx2 = list(set(idx2))
    print('eval21 : len of idx with overlapping genmap')
    print(len(idx2),1000*len(idx2))
    idx_t = combine_and_filter_new_ones(idx2)
    print('eval21 : len of idx without overlapping genmap')
    print(len(idx_t))
    tot_len = 0
    for i in idx_t:
        tot_len += i[1] - i[0]
    ...
    best2 = list(set(best2))
    print('eval22 : len of best idx with overlapping genmap')
    print(len(best2),1000*len(best2))
    idx_t = combine_and_filter_new_ones(best2)
    print('eval22 : len of best idx without overlapping genmap')
    print(len(idx_t))
    tot_len = 0
    for i in idx_t:
        tot_len += i[1] - i[0]
    ...
    """ ONLY IF BOTH """
    """ IF FISHY...CHECK ABS-REL coords relation """

    print('eval3 len of both before combining, macle,genmap')
    print(idx1,idx2)
    ...
    for i in idx1:
        pos_macle.append((i,i+len_lmers))
    for i in idx2:
        pos_genmap.append((i,i+len_lmers))
    ...
    for ms,me in pos_macle:
        ...
        if smaller_start_than_end_idx - bigger_end_than_start_idx < 1:
            continue
        for gs,ge in pos_genmap[max(0,bigger_end_than_start_idx-3):min(smaller_start_than_end_idx-3,len(pos_genmap))]:
            t1 = (ms,me)
            t2 = (gs,ge)
            if overlaps(t1,t2):
                idx.append(ms)
    ...
    print('eval3 len of both after combining, macle,genmap')
    print(idx)
    #    for gs,ge in pos_genmap:
    #        if gs > me:
    #            break
    #        if ge < ms:
    #            continue
    ...
    print('eval420 len list set idx before')
    print(len(idx))
    idx = list(set(idx))
    print('eval420 len list set idx after')
    print(len(idx))
    
    left = wanted_genome_size
    print('fill up')
    print('left',left)
    print('len idx',len(idx),1000*len(idx))
    if len(idx)*len_lmers < left:
        print('idx small')
        iii = 0
        while len(idx)*len_lmers < left and iii < len(fill_up_til_size):
            if fill_up_til_size[iii] not in idx:
                idx.append(fill_up_til_size[iii])
            iii += 1
    ...
    print(f'{len(idx)} non-overlapping candidates are processed')
