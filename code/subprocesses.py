...
def run_blast_clasp_with_retry(db, query_fasta, blast_outfile, clasp_outfile,
    ...
    # rc > 0 = application error (wrong binary, bad args) — splitting cannot fix this
    if rc > 0:
        which = 'clasp' if blast_ok else 'blast'
        if (which == 'clasp' and rc != 255) or (which == 'blast' and rc != 4):
            raise RuntimeError(
...
def makeblastdb(org):
    ...
    if not (isfile(path + "/blastdbs/{}.nin".format(org)) and isfile(path + "/blastdbs/{}.nhr".format(org)) and isfile(path + "/blastdbs/{}.nsq".format(org))):
        cmd = [
            'makeblastdb',
            '-in', f'OTHER/scr/k80san/karl/genomes/{org}.fasta',
            '-out', f'blastdbs/{org}',
            '-dbtype', 'nucl'
        ]
        run(cmd)
