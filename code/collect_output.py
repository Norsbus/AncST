...
def _set_mark_in_others(org2, positions, i_org, flags):
    """Update mark_in_others for all match positions with given [chrom, tol] flags."""
    for m in positions:
        if org2 in mark_in_others:
            if m in mark_in_others[org2]:
                mark_in_others[org2][m][i_org] = flags[:]
            else:
                mark_in_others[org2][m] = {i_org: flags[:]}
        else:
            mark_in_others[org2] = {m: {i_org: flags[:]}}
...
def check_range_and_chromos(i_org,entry,bib,org2):
    ...
    # Collect ALL match positions: regular + dups_matches
    # Both contribute to distance/chromosome checks and need mark_in_others propagation
    all_match_positions = list(entry.get('matches', {}).keys())
    if 'dups_matches' in entry:
        for k in entry['dups_matches']:
            if k != 'syntenic':
                all_match_positions.append(k)

    chromos = set()

    distances = []

    for m in all_match_positions:
        try:
    ...
    if len(chromos) <= 1:
        ...
        _set_mark_in_others(org2, all_match_positions, i_org, [1,1])
    ...
    if dis <= tolerance:
        ...
        _set_mark_in_others(org2, all_match_positions, i_org, [0,0])
    else:
        entry['meta']['multiple matches out of tolerance range'] = 1
        _set_mark_in_others(org2, all_match_positions, i_org, [0,1])
...
def check_score(score,org1,i_seq_org1,start1,end1,org2,i_seq_org2,start2,end2,strict = 0,max_score = 0):
    if len(bib[i_seq_org1]['regions']) == 0:
        ...
        if idx_in_scores1 >= len(bib[i_seq_org1]['regions']):
            raise ValueError(f"BLAST hit start1={start1} exceeds {org1} anchor {i_seq_org1} regions max={bib[i_seq_org1]['regions'][-1]} - corrupted data or mismatched candidates")
        ...
        idx_in_scores1 = max(0, idx_in_scores1 - 1) # -1 since scores have one less members of list than regions; clamp to 0
        ...
        idx_in_scores2 = max(0, idx_in_scores2 - 1)
        maxi_score = 0
    ...
    if len(bibs[org2][i_seq_org2]['regions']) == 0:
        ...
        if idx_in_scores1 >= len(bibs[org2][i_seq_org2]['regions']):
            raise ValueError(f"BLAST hit start2={start2} exceeds {org2} anchor {i_seq_org2} regions max={bibs[org2][i_seq_org2]['regions'][-1]} - corrupted data or mismatched candidates")
    ...
    if strict == 0:
        ...
        if score > max(threshold_1,threshold_2)*2:
            ...
            default_score = get_score_threshold()
            if int(threshold_1) == int(default_score) and int(threshold_2) == int(default_score):
...
def old_check_score(score,org1,i_seq_org1,start1,end1,org2,i_seq_org2,start2,end2,strict = 0,max_score = 0):
    ...
    if strict == 0:
        ...
        if score > max(threshold_1,threshold_2)*2:
            ...
            # where there is no closest one found in own genome, hence scores of 40)
            default_score = get_score_threshold()
            if threshold_1 == default_score and threshold_2 == default_score and score > max_score/10:
                return(True)
            else:
                return(False)
...
def reconcile(old_entry,new_entry,org2,i_seq_org1,inconsistencies):
    ...
    if results_org1['meta']['multiple matches on different chromosomes'] == 1 or results_org1['meta']['multiple matches out of tolerance range'] == 1:
        # Stricter scoring only applies to regular matches (dups bypass check_score).
        # If flag was set solely due to dups positions but no regular matches remain,
        # there's nothing to filter — skip the removal block.
        if len(results_org1.get('matches', {})) == 0:
...
def collect(bib,files,org,inconsistencies):
    ...
    for org2,parsed_results in parsed.items():
        ...
        for i in parsed_results:
            ...
            # Bug #3 fix: Also merge dups_matches if present in parsed_results
            # (reconcile only handles regular matches, not dups_matches)
            if 'dups_matches' in parsed_results[i]:
                if 'dups_matches' not in bib[i]['matches'][org2]:
                    bib[i]['matches'][org2]['dups_matches'] = {}
                for j, dm in parsed_results[i]['dups_matches'].items():
                    if j == 'syntenic':
                        continue  # Skip metadata key (added by finalize_aligned)
                    if j not in bib[i]['matches'][org2]['dups_matches']:
                        bib[i]['matches'][org2]['dups_matches'][j] = dm
                    elif dm['match score'] > bib[i]['matches'][org2]['dups_matches'][j]['match score']:
                        bib[i]['matches'][org2]['dups_matches'][j] = dm
...
if __name__ == "__main__":
    ...
    # Directories created by setup_directories.py -> template/make_directories.py
