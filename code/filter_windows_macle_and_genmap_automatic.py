...
def get_Cm_values(w,i):
    ...
    global_dict[org] = []
    Cms = {}
    with open(f'/scr/k80san/karl/AncST_server/AncST/utils/macle_out/{org}/{w}_{i}.txt') as f:
        for line in f:
...
def get_low_count_windows(org,genmap_out_file,out_file_name,k,e,len_window,interval,percentile,wanted_genome_size):
    ...
    how_many = int(wanted_genome_size2/len_lmers)
    y = set([xxx[1] for xxx in lll[:how_many]])
    last = lll[how_many-1][0]
    for xxx in lll[how_many:]:
        if xxx[0] <= last:
            y.add(xxx[1])
...
def get_low_count_windows_multiple_k_e(org,genmap_out_file,out_file_name,k_e,len_window,interval,percentile):
    ...
    for k,e in k_e:
        ...
        genmap_out_file = f'/scr/k80san/karl/AncST_server/AncST/utils/genmap_out/{org}/{k}_{e}.freq16'
...
def get_idx_from_macle(macle_file,half_the_window_size,pitch,percentile,global_Cm,wanted_genome_size,Cm2):
    starts = []
    Cms = []
    lll = []
    with open(f'/scr/k80san/karl/AncST_server/AncST/utils/macle_out/{org}/{half_the_window_size*2}_{pitch}.txt') as f:
        for line in f:
    ...
    best = set([xxx[1] for xxx in lll if xxx[0] >= Cm2])

    y = set([xxx[1] for xxx in lll if xxx[0] >= global_Cm])
...
if __name__ == "__main__":
    ...
    org = argv[1]
    genome_size = os.path.getsize(f'/scr/k80san/karl/AncST_server/AncST/utils/genomes/{org}.fasta')
    ...
    with open(f'/scr/k80san/karl/AncST_server/AncST/utils/metadata_genomes/{}'.format(org),'rb') as f:
    ...
    len_lmers = 1000
    interval = 100
    wanted_genome_size = genome_size*0.75
    wanted_genome_size2 = genome_size*0.2
    ...
    tot = 0
    iii = 0
    while tot < wanted_genome_size2:
        Cm_value = Cm_values[org][iii][0]
        tot += Cm_values[org][iii][1]*len_lmers
        iii+=1

    x,y = get_idx_from_macle(infile_macle,int(len_lmers/2),int(interval),percentile,Cm_value,wanted_genome_size,Cm2)
    for iii in x:
    ...
    interval = 50
    wanted_genome_size = genome_size*0.75
    wanted_genome_size = genome_size*0.3
    infile = f'/scr/k80san/karl/AncST_server/AncST/utils/genmap_out/{org}/{k}_{e}.freq16'
    x,y = get_low_count_windows(org,infile,outfile,k,e,len_lmers,interval,percentile,wanted_genome_size,wanted_genome_size2)
    for iii in x:
