...
rule checks:
    ...
    resources:
        # partner-load rule: loads aligned/{org2} for every partner; mem_mb sized
        # from the largest possible partner, not the focal wildcards.organism
        mem_mb = lambda wildcards, attempt: get_max_genome_size_mb() * MEM_MULT['checks'] * attempt
    shell:
...
rule checks_succinct:
    ...
    resources:
        # partner-load rule: loads aligned_succinct/{org2} for every partner;
        # mem_mb sized from the largest possible partner
        mem_mb = lambda wildcards, attempt: get_max_genome_size_mb() * MEM_MULT['checks_succinct'] * attempt
    shell:
