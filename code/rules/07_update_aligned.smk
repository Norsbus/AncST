...
rule collect_output:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: get_genome_size_mb(wildcards) * MEM_MULT['collect_output'] * attempt
    shell:
...
rule finalize_succinct:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: get_genome_size_mb(wildcards) * MEM_MULT['finalize_succinct'] * attempt
    shell:
        f"""cd {CODE_DIR} && {PYTHON} ./finalize_aligned.py \
